...
- [x] 1. Establish shared backend/frontend API contracts and compatibility adapters
...
- [x] 5. Refactor crawl job orchestration for persistent state and real metrics
  ...
  - [x] 5.3 Add restart-safe handling and partial progress persistence
...
- [x] 7. Build endpoint and parameter inventory as a shared substrate for scanning/manual tools
  ...
  - [x] 7.3 Seed passive scanner and manual tools from endpoint inventory
...
- [x] 9. Expand SEO and technical site audit analytics (Screaming Frog-class core)
  - [x] 9.1 Add duplicate/near-duplicate content grouping and comparison support
    - Use content hashes/similarity metrics to group duplicates and expose compare-ready payloads.
    - _Requirements: 5.3, 13.1_
  - [x] 9.2 Add link health analytics (broken links, redirects, chains, orphan candidates, depth outliers)
    - Build crawl-time or post-crawl checks with persisted status and chain metadata.
    - _Requirements: 5.4, 2.4, 13.1_
  - [x] 9.3 Extend `/api/analysis` endpoints and UI data contracts for new audit views
    - Add aggregates and page-level drilldowns with compatibility-safe responses.
    - _Requirements: 5.1, 5.2, 9.4, 11.4_

- [x] 10. Implement proxy service MVP (Burp-like interception core)
...
- [x] 12. Build proxy history UI and live interception workflow
  ...
  - [x] 12.3 Add frontend tests for proxy history filtering and live event handling
    - _Requirements: 9.2, 9.4, 15.2_

- [x] 13. Implement Repeater backend MVP (manual request replay)
...
- [x] 14. Build Repeater UI and Decoder utilities
  - [x] 14.1 Create Repeater page with raw/structured editors and response diff views
  ...
  - [x] 14.3 Add frontend tests for editor state and decoder transforms
    - _Requirements: 7.4, 15.2_

- [x] 15. Implement Intruder backend MVP (payload engine + queued fuzzing)
  ...
  - [x] 15.3 Add tests for rate limiting, stop conditions, and job resume/cancel behavior
    - _Requirements: 7.3, 14.2, 15.2_

- [x] 16. Build Intruder UI and result triage views
...
- [x] 17. Modularize security scanning engine and fully wire active scanning
  - [x] 17.1 Split passive analyzers and active check modules into pluggable scanner interfaces
  ...
  - [x] 17.2 Wire active scan execution paths and `auto_attack` behavior to real active modules
    - Ensure route/job code calls active scanners when enabled and policy-approved.
    - _Requirements: 8.1, 8.2, 8.3, 10.4_
  - [x] 17.3 Persist canonical findings with evidence/reproduction linkages
...
- [x] 18. Add safety policy enforcement and audit logging for risky actions
  - [x] 18.1 Implement workspace risk policies and confirmation gates for noisy/destructive modules
    - Enforce safe-mode checks in active scans, proxy interception edits, and Intruder runs.
    - _Requirements: 8.3, 10.3, 14.3, 14.4_
  - [x] 18.2 Add audit logging for manual/security actions and policy decisions
    - Persist auditable records for active scan starts, intercept edits, and denied actions.
    - _Requirements: 12.4, 14.1, 14.3_
  - [x] 18.3 Add tests for policy enforcement and explicit acknowledgment flows
    - _Requirements: 8.3, 14.4, 15.2_

- [x] 19. Implement findings triage APIs/UI and evidence-rich reporting exports
  - [x] 19.1 Add findings status workflow and workspace triage endpoints
  ...
  - [x] 19.2 Build export/report generators with redaction pipeline
    - Export crawl audit, security-only, and combined reports with evidence and redaction rules.
    - _Requirements: 13.1, 13.2, 13.3, 13.4_
  - [x] 19.3 Create Findings page and reporting UI flows
    - Add workspace-wide triage views, finding detail drilldowns, and report generation actions.
    - _Requirements: 9.1, 9.4, 13.1_

- [x] 20. Add run profiles, UI customization persistence, and workflow automation chaining
  - [x] 20.1 Implement run profiles for crawl/proxy/scan settings and pre-run override preview
    - Persist reusable profiles and validate risky profile settings before execution.
    - _Requirements: 10.1, 10.2, 10.3_
  - [x] 20.2 Persist UI preferences and saved views per workspace/user
  ...
  - [x] 20.3 Implement automation chaining executor (crawl -> extract -> scan -> report)
    - Add job chaining with explicit step status, failure handling, and cancellation behavior.
    - _Requirements: 10.4, 14.2, 14.3_

- [x] 21. Build performance, regression, and benchmark harnesses for release gating
  - [x] 21.1 Add backend/frontend automated regression suites for critical workflows
    - Cover crawler, migrations, API contracts, live streams, proxy/repeater/intruder core paths.
    - _Requirements: 15.2, 11.1, 11.4_
  - [x] 21.2 Implement benchmark scenarios and threshold checks in CI/local test commands
    - Define throughput/UI responsiveness/scanner performance thresholds and failure gating.
    - _Requirements: 15.1, 15.3_
  - [x] 21.3 Add competitor-comparison benchmark fixture documentation as versioned test data/code
